...
const ReputationGraph = ({ year, onDataLoaded }) => {
  ...
  if (!data) {
    ...
    return (
        <div>
            <h3>Répartition des réponses pour la question : Les éléments suivants vous semblent-ils
avoir joué un rôle dans votre recrutement ? - la réputation de la filière de formation</h3>
            <div style={{ width:"100%" }}>
            {renderChart()}
            </div>
        </div>
